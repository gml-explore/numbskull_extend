...
def create_fg():
    ...
    for i in range(weights):
        if i < 10:  # 10个需要函数化的单因子
            weight[i]["isFixed"] = True
            weight[i]["parameterize"] = True
            weight[i]["a"] = random.uniform(-5, 5)
            weight[i]["b"] = random.uniform(-5, 5)
            weight[i]["initialValue"] = random.uniform(-5, 5)  # 对于需要函数化的因子，此处initialValue没有意义。
            # 此处的initialValue对于不需要函数化的因子没有问题，对于需要函数化的因子来说不合适，应不应该
            # 单独存一个通过a,b,x计算出来的w。
        else:  # 40个不需要函数化的双因子
...
for i in range(len(fg1[0])):
    ...
    #print("weight", i, "True:",TrueWeightValue[i],"----------","Learned:", ns1.factorGraphs[0].weight_value[0][i])
        print("weight", i, "TrueA:",TrueUnaryParaA[i],"---","LearnedA:", ns1.factorGraphs[0].weight[i]['a'],"///","TrueB:",TrueUnaryParaB[i],"---","LearnedB:", ns1.factorGraphs[0].weight[i]['b'])   #上下两行输出一样，证明可以将weight写回
    # print("weight", i, "True:",TrueWeightValue[i],"----------","Learned:", fg1[0][i]['initialValue'])                       #输出同上两行
    else:
        print("weight", i, "True:",TrueBinWeightValue[i-10],"---","Learned:", ns1.factorGraphs[0].weight[i]['initialValue'])
